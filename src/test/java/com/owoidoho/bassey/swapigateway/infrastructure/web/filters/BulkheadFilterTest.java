...
public class BulkheadFilterTest {

  private static final int PROCESSING_DELAY = 50;


  @Test
  void testBulkheading() throws ServletException, IOException {
    int quota = 5;
    int extra = 5;
    ...
    for (int i = 0; i < numberOfRequests; i++) {
      int finalI = i;
      futures[i] = runAsync(
          asRunnable(() -> bulkheadFilter.doFilter(requests[finalI], responses[finalI], chain))
      );
    }
    ...
    var rejected = Stream.of(responses)
        .filter(asPredicate(response ->
            response.getStatus() == HttpStatus.TOO_MANY_REQUESTS.value() &&
                response.getContentAsString().contains("Too many concurrent requests")
        ))
        .count();
